  ...
  useEffect(() => {
    if (shopper) {
      ...
      newSocket.on('newOrderAvailable', (orderData) => {
        ...
        // Earnings are not shown to shoppers, so the alert carries the order only.
        const body = `New order #${orderData.orderNumber || orderData.orderId?.toString().slice(-6) || ''} available`;
      ...
      newSocket.on('orderUpdate', (orderData) => {
        ...
        setOrders((prev) => prev.map((order) => (order._id === orderData._id ? orderData : order)));
      ...
      newSocket.on('revisionApproved', (data) => {
        ...
        setTimeout(() => {
          alert(
            `🎉 Great news! Customer approved your revision for Order #${data.orderNumber}!\n\nNew total: ₹${displayTotal}\n\nProceed with final shopping now.`
          );
        }, 2000);
        ...
        setOrders((prev) => {
          ...
          const updated = prev.map((order) => {
            ...
            if (orderIdMatch) {
              console.log(
                'Found matching order, updating total from',
                order.totalAmount,
                'to',
                data.newTotal
              );
