...
const Dashboard = () => {
  ...
  useEffect(() => {
    // Fetch active orders and order history immediately
      ...
      if (response.data.success) {
        ...
        const sortedOrders = orders.sort((a, b) => {
          const dateA = new Date(
            a.actualDeliveryTime || a.deliveredAt || a.updatedAt || a.createdAt
          );
          const dateB = new Date(
            b.actualDeliveryTime || b.deliveredAt || b.updatedAt || b.createdAt
          );
        ...
        console.log(
          ...
          sortedOrders.map((o) => ({
            id: o._id?.slice(-8),
            date: new Date(
              o.actualDeliveryTime || o.deliveredAt || o.updatedAt || o.createdAt
            ).toLocaleDateString(),
          }))
        );
              ...
              <div className="order-details">
                <div className="shop-info">
                  <span className="shop-name">
                    🏪 {order.shopId?.name || order.shop?.name || order.shopName || 'Shop'}
                  </span>
  ...
  // Earnings intentionally hidden from shoppers: this view is order
  // history only. Commission is still recorded server-side and stays
  // visible to admins under Shopper Performance.
  const renderHistory = () => (
        ...
        {historyLoading ? (
          ...
          <div className="history-list">
            {orderHistory.map((order) => {
              // Order value shown to the shopper for cash collection
              const orderAmount =
                order.totalAmount || order.actualBill?.amount || order.orderValue?.total || 0;
      ...
      <div className="profile-settings">
        ...
        <div className="settings-grid">
          ...
          <div className="setting-item">
            ...
            <input type="text" placeholder="Add preferred areas" />
          </div>
        </div>
      </div>
    </div>
