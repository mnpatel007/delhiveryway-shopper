...
const ItemsList = ({ items }) => {
  ...
  return (
    <div className="items-list">
      ...
      {hasMoreItems && (
        <button className="toggle-items-btn" onClick={() => setShowAll(!showAll)} type="button">
          {showAll ? `Show Less` : `Show ${items.length - 3} More Items`}
        </button>
      )}
    </div>
  );
...
const OrderManagement = () => {
      ...
      prev.map((item) => {
        ...
        if (field === 'quantity') {
          ...
          if (!Number.isFinite(next) || next < 1) {
            alert(
              'You cannot decrease the quantity to zero. Instead, mark the item as Not Available.'
            );
  ...
  const submitRevision = async () => {
    try {
      ...
      const invalid = revisedItems.some(
        (it) =>
      ...
      if (invalid) {
        alert(
          'One or more items have quantity set to zero while marked Available. Please mark such items as Not Available instead.'
        );
        return;
      }
  ...
  const generateOrderSummary = (order) => {
    ...
    const permanentContactPhone =
      order.deliveryAddress?.permanentContactPhone &&
      order.deliveryAddress?.permanentContactPhone !== order.deliveryAddress?.contactPhone
      ...
      case 'accepted_by_shopper':
        return (
          <div className="action-group">
            <button className="action-btn share" onClick={() => handleShareOrder(order)}>
              📤 Share Order Details
            </button>
            <button className="action-btn primary" onClick={() => handleArrivedAtShop(order._id)}>
              I've Arrived at Shop
            </button>
            <button className="action-btn cancel" onClick={() => handleCancelOrder(order._id)}>
              ❌ Cancel Order
            </button>
          </div>
        );
      ...
      case 'shopper_at_shop':
        return (
          <div className="action-group">
            ...
            <button className="action-btn primary" onClick={() => handleStartShopping(order._id)}>
              Start Shopping
            </button>
            <button className="action-btn cancel" onClick={() => handleCancelOrder(order._id)}>
              ❌ Cancel Order
      ...
      case 'shopping_in_progress':
        return (
          <div className="action-group">
            ...
            <button className="action-btn warning" onClick={() => handleReviseOrder(order)}>
              Revise Order (Items Unavailable)
            </button>
            <button className="action-btn success" onClick={() => handleFinalShopping(order._id)}>
      ...
      case 'customer_approved_revision':
        return (
          <div className="action-group">
            ...
            <button className="action-btn primary" onClick={() => handleFinalShopping(order._id)}>
      ...
      case 'final_shopping':
        return (
          <div className="action-group">
            ...
            <button className="action-btn primary" onClick={() => handleCompleteOrder(order._id)}>
      ...
      case 'out_for_delivery':
        return (
          <div className="action-group">
            ...
            <button className="action-btn success" onClick={() => handleDelivered(order._id)}>
        ...
        <div className="revision-modal">
          ...
          <div className="revision-content">
            {revisedItems.map((item, index) => (
              <div key={item.itemId} className="revision-item">
                <div className="item-info">
                  <h4>{item.name}</h4>
                  <p>
                    Original: {item.originalQuantity} × ₹{item.originalPrice}
                  </p>
                </div>
                  ...
                  <div className="availability-control">
                    <label>
                      <input
                        ...
                        onChange={(e) =>
                          handleItemRevision(item.itemId, 'isAvailable', e.target.checked)
                        }
                      />
                      Available in Store
                    </label>
            ...
            <div className="revision-actions">
              <button className="action-btn secondary" onClick={() => setRevisionMode(false)}>
                Cancel
              </button>
              <button className="action-btn primary" onClick={submitRevision}>
      ...
      <div className="orders-header">
        <div className="header-content">
          ...
          <div className="header-controls">
            ...
            <button className="refresh-btn" onClick={fetchShopperOrders} title="Refresh orders">
              🔄 Refresh
            </button>
          </div>
        </div>
        ...
        <div className="no-orders">
          ...
          <p>
            {searchTerm
              ...
              : 'Orders will appear here once customers place them and you accept them.'}
          </p>
        </div>
      ) : (
            ...
            <div key={order._id} className="order-card">
              <div className="order-header">
                ...
                <div className="order-value">
                  <div className="total-breakdown">
                    ...
                    (order.orderValue?.originalTotal &&
                      order.orderValue?.originalTotal !== order.orderValue?.total) ||
                    ...
                    (order.revisedOrderValue && order.revisedOrderValue.total) ? (
                      <>
                        <div className="total-row">
                          ...
                          <span className="amount original">
                            ₹
                            {(order.orderValue?.originalTotal || order.totalAmount || 0).toFixed(2)}
                          </span>
                        </div>
                        <div className="total-row">
                          <span className="total-label">Revised:</span>
                          <span className="amount revised">
                            ₹
                            {(
                      ...
                      <>
                        <div className="total-row">
                          ...
                          <span className="amount no-revision">
                            ₹{(order.totalAmount || order.orderValue?.total || 0).toFixed(2)}
                          </span>
                        </div>
              ...
              <div className="order-details">
                ...
                <div className="delivery-info">
                  ...
                  <p>
                    {order.deliveryAddress?.city || 'N/A'}, {order.deliveryAddress?.zipCode || ''}
                  </p>
                  {order.deliveryAddress?.instructions && (
                    <p>
                ...
                <div className="shop-info">
                  <h4>Shop</h4>
                  <p>
                    <strong>Name:</strong> {order.shopId?.name || order.shop?.name || 'N/A'}
                  </p>
                  <p>
              ...
              <div className="order-actions">{getStatusActions(order)}</div>

              {order.revisionNote && (
                <div className="revision-status">
                  <p>
                    <strong>Revision Note:</strong> {order.revisionNote}
                  </p>
                </div>
              )}
            </div>
